...
public class MemberDao implements IMemberDao {
	...
	public int updateMember3(Map<String, String> dataMap) {
		...
		try {
			...
			/*
			 * for(String fieldName : dataMap.keySet()) { if(!"memId".equals(fieldName)) {
			 * if(!"".equals(temp)) { temp += ", "; } temp += fieldName + " = '" +
			 * dataMap.get(fieldName) + "'"; } }
			 * 
			 * String sql = "update mymember set " + temp + " where mem_id = ? "; pstmt =
			 * conn.prepareStatement(sql); pstmt.setString(1, dataMap.get("memId"));
			 * //-----------------------------------------
			 */
